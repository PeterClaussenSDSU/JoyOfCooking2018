...
- Remove matched measure column, it was there mostly for debugging and is not needed for analysis.
- Remove measure column (redundant information), the weight column suffices.
- Remove NDB_No column, it is no longer needed
- Replace the amount column with the weight column.
```{r}
master.list$Matched_Measure <- NULL
master.list$Amount <- NULL
master.list$Measure <- NULL
master.list$NDB_No <- NULL
names(master.list)[names(master.list) == 'Gm_Wgt'] <- 'Amount'
```
